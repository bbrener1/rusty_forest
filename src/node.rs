...
mod node_testing {
    ...
    fn node_test_trivial_trivial() {
        let mut root = Node::feature_root(&vec![], &vec![], &vec![][..], &vec![][..], &vec![][..], DropMode::Zeros, None, FeatureThreadPool::new(1));
        root.mads();
        root.medians();
    }
    ...
    fn node_test_trivial() {
        let mut root = Node::feature_root(&vec![vec![]],&vec![vec![]], &vec!["one".to_string()][..], &vec!["a".to_string()][..], &vec!["1".to_string()][..], DropMode::Zeros, None, FeatureThreadPool::new(1));
        root.mads();
        root.medians();
    }
